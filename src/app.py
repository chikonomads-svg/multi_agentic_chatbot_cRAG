...
# via `streamlit run`.
ROOT = Path(__file__).resolve().parents[2]  # Adjusted to include project root directly
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT / "multi_agentic_crag"))  # Fix indentation issue

from multi_agentic_crag1.src.graph import build_graph, State  # Adjusted to absolute import
from multi_agentic_crag1.src.utils.env import load_env  # Adjusted to absolute import
from multi_agentic_crag1.src.utils.loaders import load_documents_from_dir  # Adjusted to absolute import
from multi_agentic_crag1.src.vector_store import create_and_persist_faiss, load_faiss, process_folder  # Adjusted to absolute import
import shutil
from multi_agentic_crag1.src.agents.analyst import detect_and_build_plot  # Adjusted to absolute import
from multi_agentic_crag1.src.logging_config import configure_logging  # Adjusted to absolute import
...
# Removed 'Directory to ingest (local)' field from UI
        uploaded_files = st.sidebar.file_uploader("Upload files to ingest", accept_multiple_files=True)

        # If the user uploaded files via the UI, save them into a local directory and use it.
        files_dir = None
        if uploaded_files:  # Adjusting logic and fixing gaps
            files_dir = os.path.join("data", "uploads", index_name)  # Addressing indentation mismatch
        if files_dir is not None:  # Add NoneType safety check for files_dir
        ...
        ingest_path = files_dir  # Adjust indentation to avoid parsing issues
            ...
            if not ingest_path:
                ...
                if os.path.isdir(ingest_path):
                    ...
                    if vs is not None:
                        ...
                        dest = os.path.join("C:\\Users\\Komal\\OneDrive\\Desktop\\Vibe Coding\\multi_agentic_crag\\data", index_name)
                    ...
                    idx = create_and_persist_faiss(docs, f"C:\\Users\\Komal\\OneDrive\\Desktop\\Vibe Coding\\multi_agentic_crag\\data/{index_name}")
